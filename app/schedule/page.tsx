  ...
  return (
    <div className="max-w-lg mx-auto px-4 pt-5 pb-10 space-y-4">
      <div className="space-y-1">
        <h1 className="text-2xl font-bold text-gray-900">Schedule</h1>
        <p className="text-sm text-gray-500">Preview your upcoming workouts</p>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm p-3 flex items-center justify-between">
        <button
          ...
          className="p-2 rounded-lg disabled:opacity-30 hover:bg-gray-50 transition-colors"
          aria-label="Previous week"
        >
          <ChevronLeft size={20} className="text-gray-600" />
        </button>
        <div className="text-center">
          <div className="font-bold text-gray-900">Week {currentWeek} of {TOTAL_WEEKS}</div>
          <div className="text-xs text-gray-500">{phaseData.name}</div>
        </div>
        <button
        ...
        <div className="p-4 space-y-1.5">
          {workout.exercises.map((we, i) => {
            ...
            return (
              <div
                key={i}
                className="flex items-start justify-between gap-3 text-sm py-1.5 border-b border-gray-50 last:border-0"
              >
                <span className="text-gray-800 flex-1 leading-snug">{ex.name}</span>
                <span className="text-gray-400 text-xs whitespace-nowrap pt-0.5">{target}</span>
              </div>
            )
          })}
        </div>
      ) : (
        <div className="p-4 text-sm text-gray-500 leading-relaxed">{style.subtitle}</div>
      )}
    </div>
  )
}
...
function Skeleton() {
  return (
    <div className="max-w-lg mx-auto px-4 pt-5 pb-10 space-y-4">
      ...
      <div className="flex gap-2">
        {[0, 1, 2].map(i => (
          <div key={i} className="flex-1 h-12 bg-gray-200 rounded-xl animate-pulse" />
        ))}
      </div>
